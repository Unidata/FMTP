...
typedef struct VcmtpHeader {// all the types were changes to 64 to match the dessertation
	//u_int16_t	src_port;
	//u_int16_t	dest_port;
	u_int64_t 	session_id; //file identifier
	u_int64_t	seq_number;
	u_int64_t	data_len; //length of the VCMTP packet
...
struct VcmtpSenderMessage {
	int8_t		msg_type;//this changed from 32 bit to 8 bit to match jie dessertation
	//uint32_t	session_id;
	uint64_t 	data_len;//this changed to 8 bytes to match dissertation
	char       	text[256];
	//double		time_stamp;
};
