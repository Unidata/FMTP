...
uint32_t vcmtpSendv3::SendBOPMessage(uint64_t prodSize, void*metadata, unsigned metaSize)
{
    ...
    uint16_t maxMetaSize = metaSize>(VCMTP_DATA_LEN-10)?VCMTP_DATA_LEN-10:metaSize;
    const int PACKET_SIZE = VCMTP_HEADER_LEN + maxMetaSize + 10;
    ...
    memcpy(vcmtp_data+8,      &maxmetasize,   2);
    memcpy(vcmtp_data+10, meta,maxMetaSize);
	//send the bomd message
    if (numOfSentBytes+= udpsocket->SendTo(vcmtp_packet,PACKET_SIZE) < 0)
